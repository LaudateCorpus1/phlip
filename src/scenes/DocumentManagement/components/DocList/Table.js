...
import { Grid, defaultTableRowRenderer } from 'react-virtualized'
...
export default class Table extends PureComponent {
  ...
  _createHeader({ column, index }) {
    ...
    if (sortEnabled || onHeaderClick) {
      ...
      const newSortDirection = sortBy !== dataKey || sortDirection === 'DESC'
        ? 'ASC'
        : 'DESC'
    ...
    return (
      <div {...a11yProps} key={`Header-Col${index}`} className={classNames} style={style}>
        {renderedHeader}
      </div>
    )
  }
  ...
  render() {
    ...
    // Note that we specify :rowCount, :scrollbarWidth, :sortBy, and :sortDirection as properties on Grid even though
    // these have nothing to do with Grid. This is done because Grid is a pure component and won't update unless its
    // properties or state has changed. Any property that should trigger a re-render of Grid then is specified here to
    ...
    return (
      <div
        ...
        <Grid
          ...
          ref={this._setRef}
          containerStyle={{ width: '100%', maxWidth: 'unset', ...containerStyle }}
